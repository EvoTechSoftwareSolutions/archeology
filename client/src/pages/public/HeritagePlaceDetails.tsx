...
const HeritagePlaceDetails = (props: HeritagePlaceDetailsProps) => {
  ...
  const { place, loading, error, nearbyPlaces, nearbyLoading, nearbyError } =
    useHistoricalPlace(identifier as any);
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-[#F8F6F1] flex items-center justify-center">
        <div className="text-[#1C5F46] font-medium text-lg">
  ...
  if (hasRouteParam && (error || !place)) {
    return (
      <div className="min-h-screen bg-[#F8F6F1] flex flex-col items-center justify-center p-6 text-center">
        <h2 className="text-3xl font-serif text-[#3B2F1E] mb-4">
          Place Not Found
        </h2>
        <p className="text-gray-600 mb-8 max-w-md">
          {error ||
            "The heritage place you are looking for doesn't exist or has been moved."}
        </p>
        <Link
          to="/all-places"
  ...
  const resolvedPlace: HistoricalPlace = place
    ? { ...fallbackPlace, ...place }
    : { ...fallbackPlace };

  const resolvedHeroImage = resolveImageUrl(resolvedPlace.image);
...
const highlightCards: Slide[] =
  ...
  (resolvedGalleryImages.length > 0
    ? resolvedGalleryImages.map((img, idx) => ({
        ...
        description:
          img.description || resolvedPlace.description || undefined,
      }))
    : [
        {
          id: "highlight-main",
  ...
  const navigateCard = (direction: "prev" | "next") => {
    ...
    const currentIndex = allSlides.findIndex(
      (card) =>
        card.subtitle === activeCard.subtitle &&
        card.image === activeCard.image,
    );
  ...
  const drivingTimeStr =
    resolvedPlace.drivingTime || props.drivingTime || "4 HR 10 MIN";
  const walkingTimeStr =
    resolvedPlace.walkingTime || props.walkingTime || "35 HR";
  ...
  const bestTimeStr =
    resolvedPlace.recommendedDeparture || props.bestTime || "6:30 AM";
  const photoTimeStr =
    resolvedPlace.photographyTime || props.photographyTime || "5:30 PM";
  ...
  if (timelineItems.length === 0) {
    timelineItems = props.timeline?.length
      ...
      : [
          ...
          {
            year: "1687",
            title: "PRESERVED BY KINGDOMS",
            text: "Royal patrons contributed to the site expansion and ongoing rituals.",
          },
          {
            year: "1982",
            title: "UNESCO INSCRIPTION",
            text: "Sri Lanka’s heritage sites were recognized for their global value.",
          },
          {
            year: "1998",
            title: "RESTORATION",
            text: "Careful archaeological efforts helped preserve key architectural structures.",
          },
          {
            year: "TODAY",
            title: "CULTURAL LANDMARK",
            text: "Maintained as an active sanctuary and preserved heritage monument.",
          },
        ];
  }
    ...
    : [
        {
          label: "Address",
          value:
            resolvedPlace.contactAddress ||
            `${resolvedPlace.name}, ${districtName}, ${provinceName}, Sri Lanka`,
        },
        {
          label: "Administration Division",
          value:
            resolvedPlace.contactAdminPhone ||
            "+94 11 269 2840 (for prior appointments)",
        },
        {
          label: "Emergency Contact",
          value: resolvedPlace.contactEmergencyPhone || "+94 70 156 4347",
        },
        {
          label: "Official Website",
          value: resolvedPlace.contactWebsite || "www.heritage.gov.lk",
        },
        {
          label: "Inquiries Email",
          value: resolvedPlace.contactEmail || "info@heritage.gov.lk",
        },
      ];
  ...
  const essentials: EssentialItem[] = (() => {
    ...
    if (resolvedPlace.nearbyRestaurant)
      items.push({
        icon: FiCoffee,
        title: "Restaurants",
        subtitle: resolvedPlace.nearbyRestaurant,
      });
    if (resolvedPlace.nearbyHotels)
      items.push({
        icon: FiHome,
        title: "Hotels",
        subtitle: resolvedPlace.nearbyHotels,
      });
    if (resolvedPlace.nearbyFuel)
      items.push({
        icon: FiDroplet,
        title: "Fuel Stations",
        subtitle: resolvedPlace.nearbyFuel,
      });
    if (resolvedPlace.nearbyHospitals)
      items.push({
        icon: FiPlus,
        title: "Hospitals",
        subtitle: resolvedPlace.nearbyHospitals,
      });
    if (resolvedPlace.nearbyWashrooms)
      items.push({
        icon: FiWind,
        title: "Washrooms",
        subtitle: resolvedPlace.nearbyWashrooms,
      });
    if (resolvedPlace.nearbyBusStops)
      items.push({
        icon: FiTruck,
        title: "Bus Stops",
        subtitle: resolvedPlace.nearbyBusStops,
      });
    if (resolvedPlace.nearbyParking)
      items.push({
        icon: FiMapPin,
        title: "Parking",
        subtitle: resolvedPlace.nearbyParking,
      });
    if (resolvedPlace.nearbyRailway)
      items.push({
        icon: FiMap,
        title: "Railway",
        subtitle: resolvedPlace.nearbyRailway,
      });
      ...
      : props.essentials || [
          {
            icon: FiCoffee,
            title: "Restaurants",
            subtitle: "Local Cafes & Dining",
          },
          { icon: FiHome, title: "Hotels", subtitle: "Heritage Rest Houses" },
          {
            icon: FiDroplet,
            title: "Fuel Stations",
            subtitle: "National Fuel Stations",
          },
          {
            icon: FiPlus,
            title: "Hospitals",
            subtitle: "District General Hospital",
          },
          {
            icon: FiWind,
            title: "Washrooms",
            subtitle: "Visitor Center Facilities",
          },
          { icon: FiTruck, title: "Bus Stops", subtitle: "Central Bus Stand" },
          {
            icon: FiMapPin,
            title: "Parking",
            subtitle: "Public Visitor Parking",
          },
  ...
  const dressCodeStr =
    resolvedPlace.dressCode ||
    "Visitors are requested clothing should cover shoulders, arms, and knees when visiting sacred zones.";
  const photographyRulesStr =
    resolvedPlace.photographyRules ||
    "Photography allowed except inside designated inner sanctums or during sacred rituals.";
  const accessibilityStr =
    resolvedPlace.accessibility ||
    "Ground-level monument grounds and walkways offer accessible routes.";
  ...
  return (
    <div className="bg-[#F8F6F1] min-h-screen">
      ...
      <OpeningHoursContact
        ...
        openingHours={
          resolvedPlace.openingHours ||
          props.openingHours ||
          "Open Daily from 6.00AM to 6.00PM"
        }
        visitNote={
          resolvedPlace.visitNote ||
          props.visitNote ||
          "Special guided historical tours and light show displays are held during peak holiday seasons."
        }
      ...
      {nearbyLoading && (
        <div className="max-w-[1200px] mx-auto px-6 md:px-10 py-8">
          <p className="text-gray-500">Loading nearby heritage places...</p>
        </div>
      )}
